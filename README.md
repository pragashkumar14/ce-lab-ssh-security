...
4. **Check important security settings (effective values):**
   ```bash

   # Should be 'no' (password login disabled)
   sudo sshd -T | grep -i passwordauthentication

   # Should be 'without-password'
...
5. **Verify these security settings are in place:**
   ...
   - ✅ `permitrootlogin without-password` - Root can only log in with a key, never a password (synonym for `prohibit-password`; Amazon Linux 2023 default)
...
Create a **public** GitHub repository named `ce-lab-ssh-connection-security` containing:
...
**Structure:**
```
ce-lab-ssh-connection-security/
├── README.md
